...
public class Sharelink extends RPCClient {
    private Configuration conf;
    private StaticFileStorage linkStorage;
    private static final Logger L = LoggerFactory.getLogger(Sharelink.class);
    private static final int IMGCOUNT = 5;

    public Sharelink(TransceiverFactory tf, Configuration conf) {
        super(tf);
        this.conf = conf;
    ...
    public RecordSet shareImgLink(String url) throws Exception {
        ...
        if (input != null) {
            L.debug("Save img to local success,Url=" + url);
            ...
            try {
                ...
                L.error("read img error,img_name=" + img_name);
            ...
            if (image != null) {
                ...
                long scale = conf.getInt("platform.linkImgScale", 210);
...
//            SFSUtils.saveScaledImage(new ByteArrayInputStream(imgBuf), linkStorage, img_name, String.valueOf(bw), String.valueOf(bh), StringUtils.substringAfterLast(img_name, "."));
            ...
            L.debug("make img success,new url=:" + imgUrl);
            ja.add(imgUrl);
        }
        ...
        L.debug("out_recs=" + out_recs.toString(false, false));
        return out_recs;
    }
    ...
    public RecordSet shareLink(String url, String linkImagAddr) throws Exception {
        RecordSet out_recs = new RecordSet();
        String newURL = url;
        ...
        for (retriedCount = 0; retriedCount < retry_count; retriedCount++) {
            try {
                ...
                if (conn.getResponseCode() != 200 ||
                    ...
                    L.error("url:" + url + " can not allow to read!");
                    return out_recs;
                }
                ...
                if (retriedCount == retry_count) {
                    ...
                    try {
                        L.info("Sleeping " + retryIntervalSeconds + " seconds until the next retry. url=" + url);
            ...
            try {
                if (ifImgUrl(newURL)) {
                    ...
                    out_recs = shareHtmlLink(newURL, linkImagAddr);
                }
            } catch (Exception ne) {
                fetch_ret = false;
                if (retriedCount == retry_count) {
                ...
                try {
                    L.info("Sleeping " + retryIntervalSeconds + " seconds until the next retry.");
                    Thread.sleep(retryIntervalSeconds * 1000);
                } catch (InterruptedException ignore) {
                    //nothing to do
                }
    ...
    public RecordSet shareHtmlLink(String url, String linkImagAddr) throws Exception {
        RecordSet out_recs = new RecordSet();
        Record out_rec = new Record();
        // put url
        out_rec.put("url", url);
        ...
        if (result.isEmpty())
            L.error("this page is not allow to read!");
        ...
        JSONArray ja = new JSONArray();
        ja.clear();
        ja = getImgFromPage(url, getUrlHead(url) + host, pagetext, img_id, linkImagAddr);
        out_rec.put("many_img_url", ja.toString());
        if (ja.size() > 0) {
            out_rec.put("img_url", ja.get(0).toString());
        } else {
    ...
    public String getAndMakeImg(String urlHead, NodeList imgNodeList, Pattern pat, String linkImagAddr, int imgIndex, Configuration conf) throws Exception {
        String outTarget = "";
...
//                SFSUtils.saveScaledImage(new ByteArrayInputStream(imgBuf), linkStorage, img_name, String.valueOf(bw), String.valueOf(bh), StringUtils.substringAfterLast(img_name, "."));
    ...
    public JSONArray getImgFromPage(String url, String urlHead, HtmlPage page, String tagName, String linkImagAddr) throws Exception {
...
//        Configuration conf = Configuration.loadFiles(Constants.confPath).expandMacros();
        ...
        if (hasNode) {
            String img_node = getAndMakeImg(urlHead, imgNodeList, pat, linkImagAddr, imgIndex, conf);
...
//                    String target = "";
                    ...
                    if (input != null) {
                        L.debug("Save img to local success,Url=" + src);
                        ...
                        L.debug(urlHead + src + ",the size is width=" + bw + ";and height=" + bh);
                        if (bw > 150 && bh > 100) {
                            L.debug("save no:" + saveImgCount + ",old url=" + src);
                            long scale = conf.getInt("platform.linkImgScale", 210);
                            long sw = 0;
                            long sh = 0;
                            if (bw > bh) {
                            ...
                            L.debug("img_name: " + img_name);
                            L.debug("img_name_small: " + img_name_small);
//                            SFSUtils.saveScaledImage(new ByteArrayInputStream(imgBuf), linkStorage, img_name, String.valueOf(bw), String.valueOf(bh), StringUtils.substringAfterLast(img_name, "."));
                            if (linkStorage instanceof OssSFS)
                                img_name_small = "media/link/" + img_name_small;
                            SFSUtils.saveScaledImage(new ByteArrayInputStream(imgBuf), linkStorage, img_name_small, String.valueOf(sw), String.valueOf(sh), StringUtils.substringAfterLast(img_name_small, "."));
                            ...
                            String imgUrl = StringUtils.isNotBlank(img_name_small) ? String.format(conf.checkGetString("platform.linkImgUrlPattern"), img_name_small) : img_name_small;
                            L.debug("save no:" + saveImgCount + ",new url=:" + imgUrl);
                            ja.add(imgUrl);
                            saveImgCount += 1;
