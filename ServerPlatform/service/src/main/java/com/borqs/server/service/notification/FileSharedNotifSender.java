...
public class FileSharedNotifSender extends GroupNotificationSender {
    ...
    public FileSharedNotifSender(Platform p, Qiupu qiupu) {
        super(p, qiupu);
        isReplace = true;
    }
...
//    	return String.valueOf(findAppIdFromPostType((Integer)args[0]));
    ...
    public String getTitle(Object... args) {
        if (groups.isEmpty())    {
            return args[1] + "给您分享了他的"+getFileType(String.valueOf(args[0]))+":"+String.valueOf(args[2])+"";
        }
        else {
            ...
            return args[1] + "在" + groupType + StringUtils2.joinIgnoreBlank("，", groupNames) + "分享了他的"+getFileType(String.valueOf(args[0]))+":"+String.valueOf(args[2])+"";
        }
    }
    ...
    protected String getTitleHtml(Object... args) {
        if (groups.isEmpty())
            return "<a href=\"borqs://profile/details?uid=" + args[1] + "&tab=2\">" + args[2]+ "</a>给您分享了他的"+getFileType(String.valueOf(args[0]))+":"+String.valueOf(args[3])+"";
        else {
            String groupIds = StringUtils2.joinIgnoreBlank(",", groups);
            String groupType = "公共圈子";
            RecordSet recs = new RecordSet();
            ...
            return "<a href=\"borqs://profile/details?uid=" + args[1] + "&tab=2\">" + args[2] + "</a>在" + groupType + StringUtils2.joinIgnoreBlank("，", groupNames) + "分享了他的"+getFileType(String.valueOf(args[0]))+":"+String.valueOf(args[2])+"";
        }
    }
