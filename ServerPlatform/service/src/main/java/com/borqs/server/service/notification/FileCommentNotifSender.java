...
public class FileCommentNotifSender extends NotificationSender {
    private static final org.slf4j.Logger L = LoggerFactory.getLogger(FileCommentNotifSender.class);
	private String displayNames = "";
    private String nameLinks = "";

    public FileCommentNotifSender(Platform p, Qiupu qiupu) {
        super(p, qiupu);
        isReplace = true;
    }
    
    protected List<Long> getScope(String senderId, Object... args) {
        L.debug("=====NTF_FILE_COMMENT=====begin");
        List<Long> userIds = new ArrayList<Long>();
        List<Long> whoComments = new ArrayList<Long>();
        if (getSettingKey().equals(Constants.NTF_FILE_COMMENT)) {
            try {
                ...
                Logger.getLogger(FileCommentNotifSender.class.getName()).log(Level.SEVERE, null, ex);
            }
            L.debug("=====NTF_FILE_COMMENT=====join userIds"+StringUtils.join(userIds,","));
        }
        ...
        L.debug("=====NTF_FILE_COMMENT=====join userIds add ignore"+StringUtils.join(userIds,","));
        try {
        ...
        L.debug("=====NTF_FILE_COMMENT=====scope"+StringUtils.join(userIds,","));
        return userIds;
    }
   ...
   @Override
    ...
    protected String getTitle(Object... args) {
        ...
        String returnString = "";
        if (getSettingKey().equals(Constants.NTF_FILE_COMMENT)) {
        	returnString = displayNames + "评论了文件";
