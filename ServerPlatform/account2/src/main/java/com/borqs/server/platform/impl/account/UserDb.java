...
import com.borqs.server.platform.account2.PropertyEntries;
import com.borqs.server.platform.account2.User;
import com.borqs.server.platform.account2.UserHelper;
...
public class UserDb extends ConfigurableBase {
    private static final Logger L = LoggerFactory.getLogger(UserDb.class);
    // table
    ...
    public User createUser(final User user) throws SQLException {
        ...
        L.error("-------------before gen user Object-----------" + user.toString() + "--------------------------");
        String sql = UserSql.insertUser(userSR_table, user);
        L.error("-------------after gen sql -----------" + sql.toString() + "--------------------------");
    ...
    public boolean destroyUser(final long userId) {
        String sql = UserSql.destroyUser(userSR_table, userId, DateHelper.nowMillis());
