...
public class AsyncSendMail {
    private final static Logger log = LoggerFactory.getLogger(AsyncSendMail.class);
    private static AsyncSendMail _instance;    
    ...
    private AsyncSendMail(Configuration config){    	
...
//    	smtpFromUserPassword = "borqsoms";
        ...
        //-------------2012-10-11-------
        smtpFromUser = "borqs.support@borqs.com";
    	smtpFromUserPassword = "borqsbpc";
        //------------end--------------------
    ...
    synchronized public static AsyncSendMail getInstance(Configuration config){
    	if(_instance == null){
    		_instance = new AsyncSendMail(config);
    	}
    	return _instance;
    }
    ...
    public void asyncSendMail(final String title, final String to,final String username, final String content, String type, String lang){
    	asyncSendMail(title, to, username, content, true, type, lang);
    }
...
//	      	  	+ "		<a href=http://" + serverHost + "/search?q=com.borqs.qiupu target=_blank>http://" + serverHost + "/search?q=com.borqs.qiupu</a>";
	...
	public void asyncSendMail(final String title, final String to,final String username, final String content, final boolean useTemplate, final String type, final String lang){
    	log.debug("entering asyncSendMessage");
    	getDispatcher().invokeLater(new AsyncTask() {
            public void invoke() throws Exception {
            	log.debug("entering asyncSendMessage invoke method");
            	...
            	if(useTemplate)
            	{
            		mailContent = mailHead(lang) + "<pre>" + content + "</pre>" + mailBottom(to, type, lang);
            	}
    ...
    public void shutdown(){
        synchronized (AsyncSendMail.class) {
            if (shutdown) {
