...
public class PurchaseImpl extends ServiceSupport implements PurchaseService {
    ...
    Record getPartition(RecordSession session, ServiceContext ctx, String partitionId) {
        return session.selectOne("market.getPartitionByIdForPurchase", CC.map("id=>", partitionId));
    }


    Record completePromotionItem(RecordSession session, ServiceContext ctx, JsonNode promotionNode, String appMod) {
        ...
        JsonNode promotionNameNode = MLTexts.trimNode(promotionNode.get("name"), locale);
        JsonNode promotionDescNode = MLTexts.trimNode(promotionNode.get("description"), locale);
        String promotionImageUrl = promotionNode.path("promotion_image").asText();
        Record rec = null;
        if (type == PPT_PRODUCT) {
            if (StringUtils.isNotEmpty(target)) {
                Record product = getProduct(session, ctx, target, null, appMod);
                if (product != null) {
                    ...
                    rec = new Record().set("type", type).set("target", target)
                            .set("name", promotionNameNode != null ? promotionNameNode : product.getJsonNode("name"))
                            .set("description", promotionDescNode != null ? promotionDescNode : product.getJsonNode("description"))
            ...
            if (StringUtils.isNotEmpty(target)) {
                ...
                if (partition != null) {
                    ...
                    rec = new Record().set("type", type).set("target", target)
                            .set("name", promotionNameNode != null ? promotionNameNode : partition.getJsonNode("name"))
                            .set("description", promotionDescNode != null ? promotionDescNode : partition.getJsonNode("description"))
                            .set("promotion_image", promotionImageUrl);
                }
            }
        } else if (type == PPT_TAG) {
            if (StringUtils.isNotEmpty(target)) {
                String tag = ObjectUtils.toString(target);
                return new Record().set("type", type).set("target", target)
                        .set("name", promotionNameNode != null ? promotionNameNode : MLTexts.makeNode(tag, locale))
                        .set("description", promotionDescNode != null ? promotionDescNode : MLTexts.makeNode("", locale))
            ...
            return new Record().set("type", type).set("target", "")
                    .set("name", promotionNameNode != null ? promotionNameNode : MLTexts.makeNode("", locale))
                    .set("description", promotionDescNode != null ? promotionDescNode : MLTexts.makeNode("", locale))
                    .set("promotion_image", ObjectUtils.toString(promotionImageUrl));
        }
    ...
    Records getPromotions(RecordSession session, ServiceContext ctx, String appId, String categoryId, Params opts) throws Exception {
        ...
        String promotionsJson = session.selectStringValue("market.getPromotionsForPurchase", CC.map(
                "app_id=>", appId,
        ...
        for (int i = 0; i < promotionsNode.size(); i++) {
            ...
            Record promotion = completePromotionItem(session, ctx, promotionNode, appMod);
            if (promotion != null)
    ...
    private RecordsWithTotal parseVersionedProductIds(RecordSession session, ServiceContext ctx, Record partition, String appMod, int appVersion, Paging paging) {
        ...
        if (StringUtils.isEmpty(appMod)) {
            appMod = AppMods.getDefaultAppModForPurchase(
    ...
    RecordsWithTotal listProductsInPartition(RecordSession session, ServiceContext ctx, String partitionId, Params opts, Paging paging) {
        ...
        if (partition != null && !partition.isEmpty()) {
            ...
            String appMod = opts.param("app_mod").asString();
            versionedProductIds = parseVersionedProductIds(session, ctx, partition, appMod, appVersion, paging);
