...
public class PublishImpl extends ServiceSupport implements PublishService {
    ...
    Records listApps(RecordSession session, ServiceContext ctx) {
        ...
        return session.selectList("market.listAppsForPublish", RecordResultMapper.get());
    }
    ...
    Records listCategories(RecordSession session, ServiceContext ctx, String appId, boolean withPricetags) {
        Records categories = session.selectList("market.listCategoriesForPublish",
                CC.map("app_id=>", appId), RecordResultMapper.get());
    ...
    private String saveImage(FileItem fi, String id, String type) throws IOException {
        ...
        return productStorage.write(fid, content);
    }
    ...
    void updateProductWithoutGet(RecordSession session, ServiceContext ctx, Record product) throws IOException {
        ...
        if (product.isType("logo_image", CharSequence.class)) {
            ...
            logoImageUrl = saveImage(product.asFileItem("logo_image"), id, "logo");
        }
        ...
        if (product.isType("cover_image", CharSequence.class)) {
            ...
            coverImageUrl = saveImage(product.asFileItem("cover_image"), id, "cover");
        }
        ...
        if (product.isType("promotion_image", CharSequence.class)) {
            ...
            promotionImageUrl = saveImage(product.asFileItem("promotion_image"), id, "promotion");
        }
        ...
        if (product.isType("screenshot1_image", CharSequence.class)) {
            ...
            screenshot1ImageUrl = saveImage(product.asFileItem("screenshot1_image"), id, "screenshot1");
        }
        ...
        if (product.isType("screenshot2_image", CharSequence.class)) {
            ...
            screenshot2ImageUrl = saveImage(product.asFileItem("screenshot2_image"), id, "screenshot2");
        }
        ...
        if (product.isType("screenshot3_image", CharSequence.class)) {
            ...
            screenshot3ImageUrl = saveImage(product.asFileItem("screenshot3_image"), id, "screenshot3");
        }
        ...
        if (product.isType("screenshot4_image", CharSequence.class)) {
            ...
            screenshot4ImageUrl = saveImage(product.asFileItem("screenshot4_image"), id, "screenshot4");
        }
        ...
        if (product.isType("screenshot5_image", CharSequence.class)) {
            ...
            screenshot5ImageUrl = saveImage(product.asFileItem("screenshot5_image"), id, "screenshot5");
        }
    ...
    Record getProduct(RecordSession session, ServiceContext ctx, String id, boolean withVersions, boolean withAvailablePricetags) {
        Record product = session.selectOne("market.findProductByIdForPublish", CC.map("id=>", id), RecordResultMapper.get());
        if (product != null) {
            if (withVersions)
    ...
    Record getVersion(RecordSession session, ServiceContext ctx, String id, int version) {
        Record versionRec = session.selectOne("market.findVersionForPublish",
                CC.map("id=>", id, "version=>", version), RecordResultMapper.get());
    ...
    Records listVersions(RecordSession session, ServiceContext ctx, String id, boolean simple) {
        Records versions = session.selectList("market.listVersionsForPublish",
                CC.map("id=>", id, "simple=>", simple), RecordResultMapper.get());
    ...
    public static String makeProductFilename(String id, int version) {
    ...
    Record uploadProduct(RecordSession session, ServiceContext ctx, ResourceFile productFile, boolean createProduct, Params params) throws IOException, NoSuchAlgorithmException {
        ...
        FileContent logoImage = makeFileContent(params.param("logo_image").asFileItem(), productFile.readLogo(), productFile.getLogoPath());
        FileContent coverImage = makeFileContent(params.param("cover_image").asFileItem(), productFile.readCover(), productFile.getCoverPath());
        FileContent s1Image = makeFileContent(params.param("screenshot1_image").asFileItem(), productFile.readScreenshot1(), productFile.getScreenshot1Path());
        FileContent s2Image = makeFileContent(params.param("screenshot2_image").asFileItem(), productFile.readScreenshot2(), productFile.getScreenshot2Path());
        FileContent s3Image = makeFileContent(params.param("screenshot3_image").asFileItem(), productFile.readScreenshot3(), productFile.getScreenshot3Path());
        FileContent s4Image = makeFileContent(params.param("screenshot4_image").asFileItem(), productFile.readScreenshot4(), productFile.getScreenshot4Path());
        FileContent s5Image = makeFileContent(params.param("screenshot5_image").asFileItem(), productFile.readScreenshot5(), productFile.getScreenshot5Path());
        String productFilePath = productFile.getPath();
        FileContent productContent = FileContent.createWithFile(productFilePath, MimeTypeUtils.getMimeTypeByFilename(productFilePath));

        String productFileId = FilenameUtils2.changeFilenameWithoutExt(productFilePath, makeProductFilename(id, version));
        String logoImageFileId = logoImage != null ? FilenameUtils2.changeFilenameWithoutExt(logoImage.filename, makeProductImageFilename(id, "logo")) : null;
        String coverImageFileId = coverImage != null ? FilenameUtils2.changeFilenameWithoutExt(coverImage.filename, makeProductImageFilename(id, "cover")) : null;
        String s1ImageFileId = s1Image != null ? FilenameUtils2.changeFilenameWithoutExt(s1Image.filename, makeProductImageFilename(id, "screenshot1")) : null;
        String s2ImageFileId = s2Image != null ? FilenameUtils2.changeFilenameWithoutExt(s2Image.filename, makeProductImageFilename(id, "screenshot2")) : null;
        String s3ImageFileId = s3Image != null ? FilenameUtils2.changeFilenameWithoutExt(s3Image.filename, makeProductImageFilename(id, "screenshot3")) : null;
        String s4ImageFileId = s4Image != null ? FilenameUtils2.changeFilenameWithoutExt(s4Image.filename, makeProductImageFilename(id, "screenshot4")) : null;
        String s5ImageFileId = s5Image != null ? FilenameUtils2.changeFilenameWithoutExt(s5Image.filename, makeProductImageFilename(id, "screenshot5")) : null;

        String productFileUrl = productStorage.write(productFileId, productContent);
        String logoImageUrl = logoImage != null ? imageStorage.write(logoImageFileId, logoImage) : null;
        String coverImageUrl = coverImage != null ? imageStorage.write(coverImageFileId, coverImage) : null;
        String s1ImageUrl = s1Image != null ? imageStorage.write(s1ImageFileId, s1Image) : null;
        String s2ImageUrl = s2Image != null ? imageStorage.write(s2ImageFileId, s2Image) : null;
        String s3ImageUrl = s3Image != null ? imageStorage.write(s3ImageFileId, s3Image) : null;
        String s4ImageUrl = s4Image != null ? imageStorage.write(s4ImageFileId, s4Image) : null;
        String s5ImageUrl = s5Image != null ? imageStorage.write(s5ImageFileId, s5Image) : null;
        ...
        createVersionWithoutGet(session, ctx, Record.of(
                ...
                "url=>", productFileUrl,
                "file_size=>", FileUtils2.getFileSize(productFilePath),
                "file_md5=>", FileUtils2.getFileMd5(productFilePath),
