...
public class PurchaseImpl extends ServiceSupport implements PurchaseService {
    ...
    Record getProduct(RecordSession session, ServiceContext ctx, String productId, Integer version) {
        String clientLocale = ctx.getClientLocale();

        int ver0;
        if (version == null) {
            ver0 = 0; // TODO: find max version
        } else {
            ver0 = version;
        }
        ...
        if (versionedProduct != null) {
            localeSelector.selectLocale(versionedProduct, clientLocale);
    ...
    @Override
    public Record getProduct(final ServiceContext ctx, final String productId, final Integer version) {
        Validate.notNull(ctx);
        ...
        return openSession(new RecordSessionHandler<Record>() {
            ...
            public Record handle(RecordSession session) throws Exception {
                return getProduct(session, ctx, productId, version);
            }
        });
    }
