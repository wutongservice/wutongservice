...
import com.borqs.server.platform.util.NameSplitter;
...
public class NameInfo implements StringablePropertyBundle, JsonBean, Copyable<NameInfo> {
    ...
    public static NameInfo split(String fullName) {
        ...
        info.setFirstPhonetic(ObjectUtils.toString(n.getPhoneticFamilyName()));
        info.setMiddlePhonetic(ObjectUtils.toString(n.getPhoneticMiddleName()));
        info.setLastPhonetic(ObjectUtils.toString(n.getPhoneticGivenName()));
        info.setPrefix(ObjectUtils.toString(n.getPrefix()));
        info.setPostfix(ObjectUtils.toString(n.getSuffix()));
        return info;
    }

    public static double calculateSimilarity(String basic, String name) {
        // 1.0              eq
        // 0.9              eq ignore case
        // 0.8 - 0.7        name contains basic
        // 0.7 - 0.6        name contains ignore case basic
        // 0.6 - 0.5        same char count

        int sameCharCount = getSameCharCount(basic, name);
        if (sameCharCount == 0)
            return 0.0;

        if (StringUtils.equals(basic, name))
            return 1.0;
        if (StringUtils.equalsIgnoreCase(basic, name))
            return 0.9;
        if (StringUtils.contains(name, basic))
            return adjustSimilarityByLength(name.length(), 0.7, 0.8);
        if (StringUtils.containsIgnoreCase(name, basic))
            return adjustSimilarityByLength(name.length(), 0.6, 0.7);

        return adjustSimilaritySameCharCount(sameCharCount, 0.5, 0.6);
    }

    private static int getSameCharCount(String s1, String s2) {
        int n = 0;
        for (int i = 0; i < s1.length(); i++) {
            char c = s1.charAt(i);
            if (StringUtils.contains(s2, c))
                n++;
        }
        return n;
    }

    private static double adjustSimilaritySameCharCount(int sameCharCount, double min, double max) {
        final int MAX = 99;
        if (sameCharCount > MAX)
            sameCharCount = MAX;
        return min + (sameCharCount / (double)MAX) * (max - min);
    }

    private static double adjustSimilarityByLength(int nameLen, double min, double max) {
        final int MAX = 99;
        if (nameLen > MAX)
            nameLen = MAX;
        return min + ((MAX - nameLen) / (double)MAX) * (max - min);
    }
