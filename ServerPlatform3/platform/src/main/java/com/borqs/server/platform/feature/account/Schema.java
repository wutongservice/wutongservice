...
public class Schema {
    ...
    private static short parseKey(JsonNode keyNode) {
        ...
        if (keyNode.isIntegralNumber()) {
            ...
            if (StringUtils.isNumeric(s)) {
                ...
                short v = ((Number)ClassHelper.getConstant(s, -1)).shortValue();
                if (v == -1)
                    throw new IllegalArgumentException("Node 'key' value error");
                return v;
            }
        } else {
    ...
    private static void load() {
        ...
        if (s != null)
            paths.addAll(StringHelper.splitList(s, ";", true));
            ...
            if (type == Type.OBJECT) {
                ...
                if (method != null) {
                    try {
                        ...
                        throw new ServerException(E.CLASS, e, "Create new default value for %s error", clazz.getName());
                    } catch (InvocationTargetException e) {
                        throw new ServerException(E.CLASS, e.getTargetException(), "Create new default value for %s error", clazz.getName());
                    }
                } else {
                    try {
