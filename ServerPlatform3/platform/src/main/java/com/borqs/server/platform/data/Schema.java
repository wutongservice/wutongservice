...
import com.borqs.server.platform.util.ImmutableNamed;
...
public class Schema extends ImmutableNamed implements Copyable<Schema> {
    ...
    public Column addColumn(String col, int type) {
        return addColumn(col, type, null);
    }

    public Column addColumn(String col, int type, String alias) {
    ...
    public static Schema parse(String json) {
        ...
        while (iter.hasNext()) {
            ...
            if (col.isObject()) {
                ...
                schema.addColumn(cn, Values.parseType(type), alias);
            } else {
                String type = col.getValueAsText().trim().toUpperCase();
                schema.addColumn(cn, Values.parseType(type));
            }
        }
    ...
    public static Schema load(String path) {
        return parse(VfsHelper.loadText(path));
    }

    public static Schema loadClassPath(Class clazz, String f) {
        return parse(VfsHelper.loadTextInClasspath(clazz, f));
    }
    ...
    public Schema appendAliases(Class clazz, String f) {
        return f != null ? loadAliases(VfsHelper.loadTextInClasspath(clazz, f)) : null;
    }

    public Schema appendAliases(String path) {
        return path != null ? loadAliases(VfsHelper.loadText(path)) : this;
    }
    ...
    public String toString() {
        return JsonHelper.toJson(new JsonGenerateHandler() {
            @Override
            public void generate(JsonGenerator jg, Object args) throws IOException {
                jg.writeStartObject();
                ...
                for (Column c : columns.values()) {
                    ...
                    if (c.aliasExists()) {
                        jg.writeStartObject();
                        jg.writeStringField("type", Values.typeToString(c.getType()).toLowerCase());
                        jg.writeStringField("alias", c.getAlias());
                        jg.writeEndObject();
                    } else {
                        jg.writeString(Values.typeToString(c.getType()).toLowerCase());
                    }
                }
                jg.writeEndObject();
    ...
    public String toDocument() {
        ...
        buff.append("  <columns>\n");
        for (Column c : columns.values()) {
            buff.append(String.format("    <column name=\"%s\" type=\"%s\"></column>\n", StringEscapeUtils.escapeXml(c.getName()), StringEscapeUtils.escapeXml(Values.typeToString(c.getType()).toLowerCase())));
        }
        buff.append("  </columns>\n");
        buff.append("</schema>\n");
    ...
    public static class Column extends ImmutableNamed implements Copyable<Column> {
        ...
        private Column(String name, int type, String alias) {
            super(name);
            Validate.isTrue(ArrayUtils.contains(new int[]{Values.BOOLEAN, Values.INT, Values.FLOAT, Values.STRING, Values.JSON}, type));
            this.type = type;
