...
public class SQLExecutor {
    ...
    private static int executeUpdate0(Statement stmt, String sql) throws SQLException {
        if (L.isTraceEnabled())
            L.trace(sql);
        int n = 0;
        try {
    ...
    public long executeUpdate(List<String> sqls) {
        ...
        long effected = 0;
        ArrayList<SQLException2> errors = new ArrayList<SQLException2>();
        for (String db : dbs) {
            try {
    ...
    public <T> T openConnections(String[] dbs, ConnectionsHandler<T> handler) {
        ...
        try {
            ...
            throw ServerException.wrap(ErrorCode.SQL, t);
        }
    }
