...
public class AppCommentNotifMaker {
    ...
    public Notification make(Context ctx, Record opts) {
        ...
        Notification n = Notification.forSend(MakerTemplates.NOTIF_MY_APP_COMMENT, AppIds.WUTONG, "");
        n.setReplace(true);
        n.setSenderId(String.valueOf(ctx.getViewer()));
        String apkId = comment.getTarget().id;
        String uri = "borqs://application/comment?id=" + apkId;

        String tiltleHtml = getTitleHtml(ctx, apkId);
        String appName = TargetInfoFormat.ANDROID_LINK.findTargetInfo(ctx, Target.APK, new String[]{apkId}).getName();
        n.setTitle(getTitle(ctx) + appName);

        String commentapp = I18nHelper.getString("com.borqs.server.impl.notifmk.i18n.notif", locale, "profile.update.notif.commentapp");
        String s = TargetInfoFormat.ANDROID_LINK.formatTargets(ctx, 3, Target.APK, new String[]{apkId});

        n.setTitleHtml(tiltleHtml + commentapp + s);

        n.setUri(uri);
        n.setBody(comment.getMessage());
        n.setBodyHtml("");
        n.setReceiverId(getAppCommentScope(ctx, comment.getTarget()));
    ...
    public String getTitleHtml(Context ctx, String apkId) {
        List<Long> listWhoComment0;
        List<Long> listWhoComment;

        int[] comment = new int[]{Actions.COMMENT};
        Page page = new Page(0, 1000);
        Conversations converWhoComment = conversation.findByTarget(ctx, null, comment, page, Target.forApk(apkId));
     ...
     * @return
     */
    public String getAppCommentScope(Context ctx, Target target1) {
        List<Long> listAll;
