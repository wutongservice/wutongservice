...
return temp.toLowerCase();
    ...
    function expandCell(id)
    {
		...
		function activeUser()
		{
		  ...
		  var nickname = document.form1.name.value;
		  ...
		  var mutual = document.form1.mutual.checked ? true : false;

		  var login_name = document.form1.login_name.value;
		  var login_param = isEmail(login_name) ? "login_email" : "login_phone";

		  var url = "http://${host}/account/create?" + login_param + "=" + login_name
		                     + "&password=" + MD5(password) + "&display_name=" + nickname
		                     + "&gender=" + gender;

		  var fromid = "zdcitkzbfqwmx";
		  if(mutual == true)
		  {
		      fromid = document.form1.fromid.value;
		  }
		  url += "&from_id=" + fromid;
	...
	{
	    var login_name = document.form1.login_name.value;
	    var account = document.form1.borqs_account.value;
	    var pwd = document.form1.borqs_pwd.value;

	    var url = "http://${host}/account/invite_bind?borqs_account=" + account
	                  + "&borqs_pwd=" + MD5(pwd) + "&login_name=" + login_name;
...
<td>
	...
	<td align=left valign=top><input type="text" name="name" value="${name0}"></td>
	</tr>
	<tr>
	...
	<td align=left valign=top><input type="text" name="login_name" value="${login_name}" readonly></td>
	</tr>
	<tr>
	...
	<input value="激    活" ID="button"  type="button" name="activeBtn" onclick="activeUser()">
...
<td>
	<table cellpadding=1 cellspacing=1 border=0 align=left>
	<tr>
	<td align=left valign=top colspan=2>激活播思账号后，您可以免费使用播思账号为您提供的应用和便捷的服务。其中包括应用宝盒，应用管家，播思读书，OpenFace等。应用宝盒是由北京播思软件技术有限公司2011年重磅推出的一款集应用管理、应用商店、社交网络、数据备份等多功能于一体的Android客户端软件。播思读书为您提供分享和阅读图书的丰富体验。OpenFace让您和您的好友面对面无距离沟通。现在就赶快<span style="cursor:hand" onclick="download()"><U><font color=#0000ff>下载</font></U></span>体验吧！
	</td>
	</tr>
	</table>
</td>
