...
public class OperImpl extends ServiceSupport implements OperService {
    ...
    Record getPartition(RecordSession session, ServiceContext ctx, String id, boolean checkPermission) {
        Record partition = session.selectOne("market.findPartitionForOper", CC.map("id=>", id), RecordResultMapper.get());
        if (partition != null) {
            if (checkPermission) {
    ...
    @Override
    public Record setPartitionRule(final ServiceContext ctx, final String id, final String rule) {
        Validate.notNull(ctx);
        Validate.notNull(id);
        ...
        return openSession(new RecordSessionHandler<Record>() {
            @Override
            public Record handle(RecordSession session) throws Exception {
                return setPartitionRule(session, ctx, id, rule);
            }
        });
    }

    Records getActivePartitions(RecordSession session, ServiceContext ctx, String appId, String categoryId) {
        Records activePartitions = session.selectList("market.listActivePartitionsForOper", CC.map(
                "app_id=>", appId,
                "category_id=>", categoryId
        ), RecordResultMapper.get());
        //localeSelector.selectLocale(activePartitions, ctx);
        urlCompleter.completeUrl(activePartitions);
        return activePartitions;
    }

    @Override
    public Records getActivePartitions(final ServiceContext ctx, final String appId, final String categoryId) {
        Validate.notNull(ctx);
        Validate.notNull(appId);
        Validate.notNull(categoryId);
        ...
        return openSession(new RecordSessionHandler<Records>() {
            @Override
            public Records handle(RecordSession session) throws Exception {
                return getActivePartitions(session, ctx, appId, categoryId);
            }
        });
    }

    Records activePartition(RecordSession session, ServiceContext ctx, String appId, String categoryId, String partitionId, int status) {
        if (status == PTS_DEACTIVE) {
            session.update("market.setActivePartitionForOper", CC.map(
                    "status=>", PTS_DEACTIVE,
                    "id=>", partitionId,
                    "app_id=>", appId,
                    "category_id=>", categoryId
            ...
            session.update("market.clearActivePartitionForOper", CC.map(
                    "app_id=>", appId,
                    "category_id=>", categoryId,
    ...
    @Override
    public Records activePartition(final ServiceContext ctx, final String appId, final String categoryId, final String partitionId, final int status) {
        Validate.notNull(ctx);
        Validate.notNull(appId);
        Validate.notNull(categoryId);
        Validate.notNull(partitionId);
        Validate.isTrue(status >= 0 && status <= ACTIVE_PARTITION_COUNTS);

        return openSession(new RecordSessionHandler<Records>() {
            @Override
            public Records handle(RecordSession session) throws Exception {
                return activePartition(session, ctx, appId, categoryId, partitionId, status);
            }
        });
    }
