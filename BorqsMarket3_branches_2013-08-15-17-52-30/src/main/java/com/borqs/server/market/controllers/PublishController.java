...
//    }
    ...
    public ModelAndView showProductStatPage(ServiceContext ctx, HttpServletRequest req, HttpServletResponse resp,
        ...
        availableStatTypes.add(new Record()
                .set("id", STAT_PURCHASE_COUNT)
                .set("name", SpringMessage.get("publish_productStat.text.purchaseCount", ctx)));
        availableStatTypes.add(new Record()
                .set("id", STAT_DOWNLOAD_COUNT)
                .set("name", SpringMessage.get("publish_productStat.text.downloadCount", ctx)));
