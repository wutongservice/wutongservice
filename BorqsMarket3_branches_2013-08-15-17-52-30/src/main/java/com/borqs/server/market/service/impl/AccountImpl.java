...
public class AccountImpl extends ServiceSupport implements AccountService {
    ...
    Record signup(RecordSession session, ServiceContext ctx, Record account) {
    ...
    Record activeAccount(RecordSession session, ServiceContext ctx, Params params) {
        long now = DateTimeUtils.nowMillis();
        ...
        if (account == null) {
            ...
            if (params.hasParam("google_id")) {
                session.insert("market.createAccount", CC.map(
                        ...
                        "avatar_image=>", params.param("avatar_image").asString(),
