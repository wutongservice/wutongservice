...
$SUDOEXP $PASS "sed 's/development/production/g' notifications_php_web/bmb.conf.bak | sed 's/\/home\/liuchunrong\/workspace\/notifications/$CMHOME/g' > notifications_php_web/bmb.conf"
...
$SUDOEXP $PASS "sed 's/local.properties/product.properties/g' notifications-server/conf/applicationContext.xml.bak |sed 's/file:./file:$CMHOME\/notifications-server/g' > notifications-server/conf/applicationContext.xml"
...
$SUDOEXP $PASS "service httpd restart" -
