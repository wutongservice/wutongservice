...
// private static final String TAG = "QueuedThreadPool";
 ...
 private int _maxIdleTimeMs=60000*10;//for save battery, increase to 120 seconds
 ...
 {
     _name="qtp"+__id++;
     log.debug("create QueuedThreadPool");
 }
 /* ------------------------------------------------------------------- */
 /* Construct
  */
 ...
 {  
     ...
     if (job instanceof NotifTask) {
         log.trace("Dispatch job: " + ((NotifTask)job).getJsonString());
         for (int i = 0; i < _jobs.length; i++) {
             if (_jobs[i] instanceof NotifTask) {
                 log.trace("In Thread Poll " + i + ": " + ((NotifTask) _jobs[i]).getJsonString());
             }
         }
     }
     return true;
 ...
 {   
     ...
     if (_threads.size()>0)
    	 log.debug( _threads.size()+" threads could not be stopped");
         ...
         try
         {
             while (isRunning())
             {
                 if (job!=null)
                 {
                     ...
                     if (todo instanceof NotifTask) {
                         log.trace("Run job: " + ((NotifTask)todo).getJsonString());
                         for (int i = 0; i < _jobs.length; i++) {
                             if (_jobs[i] instanceof NotifTask) {
                                 log.trace("In Thread Poll " + i + ": " + ((NotifTask) _jobs[i]).getJsonString());
                             }
