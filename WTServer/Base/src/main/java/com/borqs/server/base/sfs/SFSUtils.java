...
public class SFSUtils {
    ...
    public static void saveToOSS(OssSFS sfs, String file, long contentLength, InputStream is, String dispositionName) throws Exception {
        ...
        if (StringUtils.isNotBlank(dispositionName)) {
            objectMeta.setContentDisposition("attachment; filename=\"" + dispositionName + "\"");
        }
    ...
    public static void saveToOSS(OssSFS sfs, String file, long contentLength, InputStream is) throws Exception {
        saveToOSS(sfs, file, contentLength, is, "");
    }
    ...
    public static void saveUpload(FileItem fileItem, StaticFileStorage sfs, String file, String dispositionName) {
        Validate.notNull(sfs);
        Validate.notNull(fileItem);

        if (sfs instanceof OssSFS) {
            try {
                saveToOSS((OssSFS) sfs, file, fileItem.getSize(), fileItem.getInputStream(), dispositionName);
            } catch (Exception e) {
