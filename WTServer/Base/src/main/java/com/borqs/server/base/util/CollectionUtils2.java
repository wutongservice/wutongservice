...
public class CollectionUtils2 {
    ...
    public static <K, VE> Map<K, VE[]> listValueMapToArrayValueMap(Map<K, ? extends Collection<VE>> m, Class<VE> veClass) {
        if (m == null)
            return null;


        Map<K, VE[]> r = (Map)ClassUtils2.newInstance(m.getClass());
        for (Map.Entry<K, ? extends Collection<VE>> e : m.entrySet()) {
            Collection<VE> l = e.getValue();
            VE[] a;
            if (l == null) {
                a = null;
            } else {
                a = (VE[]) Array.newInstance(veClass, l.size());
                a = l.toArray(a);
            }
            r.put(e.getKey(), a);
        }
    ...
    public static <K, V> Map<K, V> fillMissingWithValue(Map<K, V> m, K[] keys, V def) {
        if (m != null) {
            for (K k : keys) {
                if (!m.containsKey(k))
                    m.put(k, def);
