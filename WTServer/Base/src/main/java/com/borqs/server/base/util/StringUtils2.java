...
public class StringUtils2 {
    ...
    public static Set<Long> splitIntSet(String s, String sep) {
        LinkedHashSet<Long> set = new LinkedHashSet<Long>();
        for (String e : StringUtils.split(s, sep))  {
            String e0 = StringUtils.strip(e);
            if (!StringUtils.isEmpty(e0))
                set.add(Long.parseLong(e0));
        }
        return set;
    }
...
//        boolean result = m.find();
    ...
    public static String addIntToSet(String set, long id) {
        Set<Long> s = splitIntSet(set, ",");
        s.add(id);
        return StringUtils.join(s, ",");
    }

    public static String removeIntFromSet(String set, long id) {
        Set<Long> s = splitIntSet(set, ",");
        s.remove(id);
        return StringUtils.join(s, ",");
    }

    public static String getSetStartN(String set, int n) {
        String[] a = splitArray(set, ",", true);
        ArrayList<String> l = new ArrayList<String>();
