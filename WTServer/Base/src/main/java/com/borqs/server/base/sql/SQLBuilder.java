...
public class SQLBuilder {
    ...
    public static String union(Object... sqls) {
        return union(Arrays.asList(sqls));
    }

    public static String union(Collection sqls) {
        ArrayList<String> sqls0 = new ArrayList<String>();
        for (Object sql : sqls) {
            String sql0 = ObjectUtils.toString(sql);
            if (!sql0.isEmpty())
                sqls0.add(sql0);
        }
        if (sqls.isEmpty()) {
            return "";
        } else if (sqls0.size() == 1) {
            return sqls0.get(0);
        } else {
            for (int i = 0; i < sqls.size(); i++) {
                sqls0.set(i, "(" + sqls0.get(i) + ")");
            }
            return StringUtils.join(sqls0, " UNION ");
        }
    }
    ...
    public static class Select extends SQLBuilder {
        ...
        public Select orderBy(String col1, String ascOrDesc1, String col2, String ascOrDesc2) {
            buffer.append(" ORDER BY ").append(getAlias(col1)).append(" ").append(ascOrDesc1);
            buffer.append(",  ").append(getAlias(col2)).append(" ").append(ascOrDesc2);
            return this;
        }
