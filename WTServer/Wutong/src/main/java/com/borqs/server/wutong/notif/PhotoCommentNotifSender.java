...
public class PhotoCommentNotifSender extends NotificationSender {
    private static final Logger L = Logger.getLogger(PhotoCommentNotifSender.class);
	private String displayNames = "";
    private String nameLinks = "";
    private ArrayList<Long> toIds = new ArrayList<Long>();

    public PhotoCommentNotifSender() {
        super();
        isReplace = true;
    }
    
    protected List<Long> getScope(Context ctx, String senderId, Object... args) {
        L.debug(ctx, "=====NTF_PHOTO_COMMENT=====begin");
        List<Long> userIds = new ArrayList<Long>();
        List<Long> whoComments = new ArrayList<Long>();
        if (getSettingKey(ctx).equals(Constants.NTF_PHOTO_COMMENT)) {
            ...
            RecordSet conversation_users = conversation.getConversation(ctx, Constants.PHOTO_OBJECT, (String) args[0], reasons, 0, 0, 100);
                ...
                RecordSet conversation_users1 = conversation.getConversation(ctx, Constants.PHOTO_OBJECT, (String) args[0], reasons1, 0, 0, 100);
            ...
            L.debug(ctx, "=====NTF_PHOTO_COMMENT=====join userIds"+StringUtils.join(userIds,","));
        }
        ...
        L.debug(ctx, "=====NTF_PHOTO_COMMENT=====join userIds add ignore"+StringUtils.join(userIds,","));
        ...
        L.debug(ctx, "=====NTF_PHOTO_COMMENT=====scope"+StringUtils.join(userIds,","));
        return userIds;
    }
   ...
   @Override
    ...
    protected String getTitle(Context ctx, String lang, Object... args) {
        ...
        String returnString = "";
        if (getSettingKey(ctx).equals(Constants.NTF_PHOTO_COMMENT)) {
        	returnString = displayNames + Constants.getBundleStringByLang(lang, "photo.comment.notif");
    ...
    protected String getUri(Context ctx, Object... args) {
        return "borqs://photo/comment?id="+(String)args[0] + "&comment_id=" + (String)args[1];
    }
    ...
    protected String getTitleHtml(Context ctx, String lang, Object... args) {
        ...
        if (getSettingKey(ctx).equals(Constants.NTF_PHOTO_COMMENT)) {
            ...
            //您"+act+"的
        	returnString = nameLinks + Constants.getBundleStringByLang(lang, "photo.comment.notif");
            if(StringUtils.isNotBlank(message))
            {
