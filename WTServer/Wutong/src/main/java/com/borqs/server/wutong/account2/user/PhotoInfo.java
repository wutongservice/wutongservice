...
public class PhotoInfo implements StringablePropertyBundle, JsonBean, Copyable<PhotoInfo> {
    ...
    public void addUrlPrefix(String prefix) {
        ...
        middleUrl = StringUtils2.addPrefix(middleUrl, prefix, "%s/", null, true);
        smallUrl = StringUtils2.addPrefix(smallUrl, prefix,"%s/", null,true);
        largeUrl = StringUtils2.addPrefix(largeUrl, prefix,"%s/",null, true);
    }
    public void addDefualtUrlPrefix(String prefix) {
        prefix = StringUtils.trimToEmpty(prefix);
        if (!prefix.endsWith("/"))
            prefix += "/";

        middleUrl = StringUtils2.addPrefix(middleUrl, prefix,"%s/","1.gif", true);
        smallUrl = StringUtils2.addPrefix(smallUrl, prefix,"%s/","1.gif", true);
        largeUrl = StringUtils2.addPrefix(largeUrl, prefix,"%s/","1.gif", true);
    }
