...
public class CourseImpl implements CourseLogic {

    public final Schema course_schema = Schema.loadClassPath(CourseImpl.class, "course.schema");
    public final Schema course_applicant_schema = Schema.loadClassPath(CourseImpl.class, "course_applicant.schema");
    public final Schema questionnaire_record_schema = Schema.loadClassPath(CourseImpl.class, "questionnaire_record.schema");
    ...
    private Record formatFileUrl(Record rec) {
        ...
        if (rec.getString("attachment_real_name").length() > 3) {
            rec.put("file_url", String.format(GlobalConfig.get().checkGetString("platform.fileUrlPattern") + bucketName + "/" + bucketName_static_file_key + "elearning/" + rec.getString("attachment_real_name").replace(".pdf",".swf")));
        } else {
            rec.put("file_url", "");
        }
        return rec;
    ...
    public boolean updateCourse(QueryParams qp, boolean has_attachment) {
        ...
        String video_name = replaceE(qp.getString("video_name", ""));
        ...
        sql_buf.append("video_name='" + video_name + "', ");
